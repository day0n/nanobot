...
class ExecutorHooks:
    ...
    on_workflow_event: Callable[[Any], Awaitable[None]] | None = None
...
class AgentExecutor:
    ...
    async def _consume_workflow_stream(
        ...
        All interpretation logic lives in the tool-provided callbacks
        (``interpret_event``, ``make_sse_event``).  This method is a pure
        pipeline: iterate events -> forward SSE -> check termination.
        """
        ...
        try:
            async for raw_event in wf_exec.event_stream:
                ...
                # 2. Let the tool interpret the event
                if wf_exec.interpret_event:
                    result_str = wf_exec.interpret_event(raw_event)
        ...
        # finish_flow/flow_killed -> _event_stream breaks -> async for ends
        # -> result_str is still None -> fall through to default_result
        return result_str if result_str is not None else wf_exec.default_result
