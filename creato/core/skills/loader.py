...
class SkillsLoader:
    ...
    def filtered(self, allowed_names: list[str] | tuple[str, ...]) -> SkillsLoader:
        """Return a lightweight copy that only exposes the named skills.

        The returned loader shares the same parsed SkillInfo objects (no
        disk I/O) but all public methods — ``load_skill``, ``list_skills``,
        ``build_skills_summary``, ``get_always_skills`` — only see the
        allowed subset.
        """
        clone = SkillsLoader.__new__(SkillsLoader)
        clone.builtin_skills = self.builtin_skills
        allowed = set(allowed_names)
        clone._skills = {k: v for k, v in self._skills.items() if k in allowed}
        return clone
