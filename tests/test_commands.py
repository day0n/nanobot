...
from creato.cli.commands import _make_provider, app
from creato.config.schema import Config
from creato.providers.litellm_provider import LiteLLMProvider
from creato.providers.openai_codex_provider import _strip_model_prefix
...
def mock_paths():
    ...
    with patch("creato.config.loader.get_config_path") as mock_cp, \
         patch("creato.config.loader.save_config") as mock_sc, \
         patch("creato.config.loader.load_config") as mock_lc, \
         patch("creato.cli.commands.get_workspace_path") as mock_ws:
...
def test_onboard_fresh_install(mock_paths):
    ...
    assert "creato is ready" in result.stdout
...
def test_onboard_existing_workspace_safe_create(mock_paths):
    """Workspace exists — should not recreate runtime root."""
    config_file, workspace_dir, _ = mock_paths
...
def test_make_provider_passes_extra_headers_to_custom_provider():
    ...
    with patch("creato.providers.custom_provider.AsyncOpenAI") as mock_async_openai:
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("creato.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("creato.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
