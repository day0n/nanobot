...
from creato.channels.email import EmailChannel
from creato.channels.email import EmailConfig
...
def test_fetch_new_messages_parses_unseen_and_marks_seen(monkeypatch) -> None:
    ...
    fake = FakeIMAP()
    monkeypatch.setattr("creato.channels.email.imaplib.IMAP4_SSL", lambda _h, _p: fake)
...
def test_fetch_new_messages_retries_once_when_imap_connection_goes_stale(monkeypatch) -> None:
    ...
    monkeypatch.setattr("creato.channels.email.imaplib.IMAP4_SSL", _factory)
...
def test_fetch_new_messages_skips_missing_mailbox(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "creato.channels.email.imaplib.IMAP4_SSL",
...
async def test_send_uses_smtp_and_reply_subject(monkeypatch) -> None:
    ...
    monkeypatch.setattr("creato.channels.email.smtplib.SMTP", _smtp_factory)
