...
async def test_exec_blocks_curl_metadata():
    tool = ExecTool()
    with patch("creato.security.network.socket.getaddrinfo", _fake_resolve_private):
...
async def test_exec_blocks_wget_localhost():
    tool = ExecTool()
    with patch("creato.security.network.socket.getaddrinfo", _fake_resolve_localhost):
...
async def test_exec_allows_curl_to_public_url():
    ...
    tool = ExecTool()
    with patch("creato.security.network.socket.getaddrinfo", _fake_resolve_public):
