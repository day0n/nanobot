...
import creato.channels.matrix as matrix_module
from creato.bus.events import OutboundMessage
from creato.bus.queue import MessageBus
from creato.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from creato.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("creato.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "creato.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("creato.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "creato.channels.matrix.asyncio.create_task", _fake_create_task
    )
