...
3. 每套方案都要写清楚：
   ...
   - 哪些节点可选，不是必选
4. 给“最小改造补丁”：
   - `add_node` / `remove_node` / `reconnect` / `update_node_model`
...
- Handy：`assembleNow`, `stickyNodesNode`（仅辅助，不放执行主链路）
...
- 基本规则：同类型输出接同类型输入
  ...
  - `video -> video`
  - `audio -> audio`
- `videoToVideo` 的 `subject/style` 输入可接图片（属于 image 类输入）。
...
- 分镜短片：`textInput -> textGenerator -> scriptSplit -> imageMaker -> videoMaker`
- 视频改造：`videoInput + textInput -> videoToVideo -> videoUpscaler`
- 视频口播：`textInput -> textToSpeech`, `videoInput + audio -> videoLipSync`
- 图片口播：`textInput -> textToSpeech`, `imageInput + audio + text -> imageAudioToVideo`
- 商品图精修：`imageInput -> backgroundEditor -> relight -> imageUpscaler`
