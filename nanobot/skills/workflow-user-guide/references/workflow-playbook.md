...
在回答非平凡场景时，应先读 `prompt-and-workflow-guide.md` 完成业务反推；本文件用于把反推结果落成可执行链路，不要跳过前一步直接套模板。
...
- 再拆能力：
  ...
  - 角度图需要视角变化
- 标准结构：
  - `textGenerator` 先规划多图职能
  - `scriptSplit` 拆成单图描述
  - 然后进入 `imageMaker` / `imageToImage` / `backgroundEditor` / `relight` / `imageAngleControl`
...
- 再判断输入：
  - 已有视频：`videoLipSync`
  - 只有图片且速度优先：`imageAudioToVideo`
  - 只有图片但还想有镜头运动：`videoMaker -> videoLipSync`
...
- 标准结构优先：
  - `textGenerator` 先写分镜
  - `scriptSplit` 拆镜头
  - 再进入 `imageMaker` / `videoMaker` / `textToVideo`
